...
class CNNBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, bn_act=True, stride=1, padding=1) -> None:
    ...
    def _make(self, in_channel_1):
        ...
        for _ in range(self.num_repeats):
            layers.append(nn.Sequential(
                CNNBlock(in_channel_1, in_channel_1//2, kernel_size=1, stride=1, padding=0),
                CNNBlock(in_channel_1//2, in_channel_1, kernel_size=3, stride=1, padding=1))
            )
...
class ResBlockB(nn.Module):
    def __init__(self, in_channel_1, use_residual=True) -> None:
        super(ResBlockA, self).__init__()  
        self.convA = CNNBlock(in_channel_1, in_channel_1//2, kernel_size=1, stride=1, padding=0)
        self.convB = CNNBlock(in_channel_1//2, in_channel_1, kernel_size=3, stride=1, padding=1)
        self.convC = CNNBlock(in_channel_1//2, in_channel_1, kernel_size=3, stride=1, padding=1)
        self.use_residual = use_residual
    def forward(self, x):
        if self.use_residual:
            return self.convC(self.convB(self.convA(x))) + x 
        else:
            return self.convC(self.convB(self.convA(x)))
...
class YOLOv3(nn.Module):
    ...
    def _create_backbone(self):
        ...
        for l in CONFIG_BACKBONE:
            if l[0] == "C":
                ...
                layers.append(CNNBlock(in_chan, l[1], l[2], l[3], padding=1))
                in_chan = l[1]
            elif l[0] == "B":
    ...
    def _create_heads(self, in_chan):
        ...
        for l in CONFIG_NECK:
            if l[0] == "C":
                l.append(CNNBlock(in_chan, l[1], l[2], l[3], padding=1))
                in_chan = l[1]
            elif l[0] == "U":
